...
export const routeFromPath=(path=window.location.pathname)=>{const p=String(path).replace(/^\/app\/?/,'').replace(/\/$/,'');if(!p)return ROUTES.home;const parts=p.split('/');const base=parts[0];if(base==='quiz'&&parts[1])return `quiz:${parts[1]}`;return Object.values(ROUTES).includes(base)?base:ROUTES.home};
